...
$helloWorkCandidates = @(Get-HelloWorkCardCandidates -Html $helloWorkCard -SearchUrl "https://www.hellowork.com/fr-fr/emploi/recherche.html?k=crm%20analyst" -Query "crm analyst" -Stats $helloWorkStats)
Assert-Fixture -Condition ($helloWorkCandidates.Count -eq 1) -Message "Expected one HelloWork card candidate."
Assert-Fixture -Condition ($helloWorkCandidates[0].Title -eq "CRM Analyst H/F") -Message "Expected HelloWork card title mapping."
...
Assert-Fixture -Condition ($helloWorkMetadata.Title -eq "CRM Analyst H/F") -Message "Expected HelloWork detail title mapping."
...
Assert-Fixture -Condition ($helloWorkMetadata.Description -match "Braze") -Message "Expected HelloWork detail description mapping."
...
$dedupeMatch = Get-JobMatch -Title "CRM Analyst" -Text "Braze HubSpot segmentation customer journey"
$dedupeRows = @(
    (New-JobResult -Title "CRM Analyst H/F" -CompanyName "Example Retailer" -JobLocation "Paris - 75" -ContractType "CDI" -MatchScore $dedupeMatch.Score -MatchLevel $dedupeMatch.Level -MatchedKeywords $dedupeMatch.Keywords -Url "https://www.hellowork.com/fr-fr/emplois/123.html" -Platform "HelloWork" -PublishedAt ([DateTimeOffset]::Now) -SourceText "Braze HubSpot"),
    (New-JobResult -Title "CRM Analyst F/H" -CompanyName "Example Retailer" -JobLocation "75 - Paris" -ContractType "CDI" -MatchScore $dedupeMatch.Score -MatchLevel $dedupeMatch.Level -MatchedKeywords $dedupeMatch.Keywords -Url "https://www.apec.fr/candidat/recherche-emploi.html/emploi/detail-offre/123W" -Platform "APEC" -PublishedAt ([DateTimeOffset]::Now) -SourceText "Braze HubSpot")
)
